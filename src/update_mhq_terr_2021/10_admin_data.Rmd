...
visits_mhq_terr <- visits_mhq_terr %>%
    filter(!recording_givid %in% dubbele_invoer)
...
reference_points_2021 <- su_2021 %>%
    ...
    mutate(point_code = ifelse(sampling_unit_code %in% c("326965_2160", "326965_2190_overig"), "326965_1", point_code),
           point_code = ifelse(sampling_unit_code %in% c("2447382_6410_mo"), "2447382_1", point_code),
           point_code = ifelse(sampling_unit_code %in% c("3351638_6410_mo"), "3351638_1", point_code))
...
assessments_admin_basic <- assessments_admin_basic %>%
    mutate(assessment_date = if_else(db_ref == "311262" & assessment_date == "2016-01-01", as.Date("2015-07-28"), assessment_date),
           assessment_date = if_else(db_ref == "300649", as.Date("2016-08-31"), assessment_date),
           assessment_date = if_else(db_ref == "1276081", as.Date("2020-08-21"), assessment_date),
           assessment_date = if_else(db_ref == "358577", as.Date("2020-08-23"), assessment_date)) %>%
    filter(db_ref != "8557745")

visits_mhq_terr_wide <- visits_mhq_terr_wide %>%
    mutate(db_ref = ifelse(db_ref == "11413617", "dh_01", db_ref)) %>%
    filter(db_ref != "2534622")
