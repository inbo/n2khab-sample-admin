...
su_anb_2021 <- read.csv2("../../output/samplingunits_prioritair_anb_versie2021-02-22.csv") %>%
    ...
    mutate(sampling_unit_code = ifelse(sampling_unit_code == "TRUE", str_c(grts_ranking_draw, "_", type_target), sampling_unit_code))

su_duinen_2160_2180 <- read.csv2("../../data/design_cycle2/mhq_terr_cyclus1_2160_2180_inbo.csv") %>%
    mutate(habitattype = as.character(habitattype))

su_duinen_cyclus2 <- read.csv2("../../data/design_cycle2/mhq_terr_cyclus2_duinen_inbo.csv") %>%
    filter(type_meetpunt == "nieuw meetpunt")

su_terr_cyclus2 <- read.csv2("../../data/design_cycle2/mhq_terr_cyclus2_inbo.csv") %>%
    filter(type_meetpunt == "nieuw meetpunt")

su_anb_2022 <- read.csv2("../../data/design_cycle2/mhq_terr_cyclus2_anb.csv") %>%
    filter(type_meetpunt == "nieuw meetpunt")

su_2022 <- su_duinen_2160_2180 %>%
    bind_rows(su_duinen_cyclus2) %>%
    bind_rows(su_terr_cyclus2) %>%
    bind_rows(su_anb_2022) %>%
    rename(type_target = habitattype, sac = sbz) %>%
    anti_join(su_2021, by = c("point_code", "type_target")) %>%
    anti_join(su_anb_2021, by = c("point_code", "type_target")) %>%
    distinct(point_code, grts_ranking_draw, sac, type_target,  x, y) %>%
...
```{r, eval = FALSE}
assessments_admin <- read_csv2(file.path(fileman_up("n2khab-mhq-data"), "raw/voortgang/voortgang_mhq_terrestrisch.csv"),
   trim_ws = TRUE)
...
assessments_admin <- assessments_admin %>%
    ...
    select(-starts_with("...")) %>%
    mutate(legacy_site = legacy_site == "WAAR",
           reassessment_1 = reassessment_1 == "WAAR",
           relocation = relocation == "WAAR",
...
assessments_admin_anb <- read_vc(root = path, file = "sample_status") 
...
INBOVEG contains the actual collected data including the coordinates. Only the assessments untill 2022 are imported.
...
header_mhq_terr <- read_vc(root = path, file = "header_mhq_terr") %>%
    ...
    filter(survey != "N2000_6510_droogte")
...
visits_mhq_terr_wide <- visits_mhq_terr_wide %>%
    ...
    mutate(n_db_ref = n()) %>%
    ungroup() %>%
    mutate(reassessed = n_db_ref > 1 & year(assessment_date) >= 2022)

reassessments <- visits_mhq_terr_wide %>%
    filter(reassessed)

check_measurements_doubles <- visits_mhq_terr_wide %>%
    filter(n_visits > 1) %>%
    anti_join(reassessments, by = "db_ref") %>%
    arrange(db_ref_old)
...
```{r}
path <- file.path(fileman_up("n2khab-mhq-data"), "raw/coordinates_inbo")

coordinates_2021_orig <- read_csv2(file.path(path, "meetpunten2021_coords.csv"))
...
## Add selected sampling units 2021 and 2022

```{r}

# voor sommige nieuwe meetpunten bestaan er al meetpunten met deezelfde point_code: PINK PQ's --> point_code aanpassen
...
reference_points_2021 <- su_2021 %>%
    ...
    left_join(select(mhq_terr_refpoints, point_code, grts_ranking_draw, is_centroid, legacy_site), by = "point_code", suffix = c("_new", "_old")) %>%
    mutate(point_code = ifelse(!is.na(is_centroid_old) & !is_centroid_old,
                               str_c(str_remove(point_code, "_\\d"), "_2"),
                               point_code)) %>%
    select(sampling_unit_code, point_code, grts_ranking, is_centroid = is_centroid_new, grts_ranking_draw = grts_ranking_draw_new, legacy_site = legacy_site_new, x, y, sac, db_ref, type_target)
...
visits_mhq_terr_wide <- visits_mhq_terr_wide %>%
    mutate(db_ref = ifelse(db_ref == "11413617", "dh_01", db_ref)) 

visits_mhq_terr_wide <- visits_mhq_terr_wide %>%
    mutate(year = year(assessment_date))

assessments_admin_basic <- assessments_admin_basic %>%
    mutate(year = year(assessment_date))

coordinates_2021 <- coordinates_2021 %>%
    mutate(year = 2021)
    
assessments_admin_inboveg_measured <- assessments_admin_basic %>%
    filter(fieldwork_team == "inbo") %>%
    filter(lsvi_measurement) %>%
    inner_join(visits_mhq_terr_wide, by = c("db_ref", "year"), suffix = c("_admin", "_inboveg")) 

assessments_admin_notinboveg_measured <- assessments_admin_basic %>%
    filter(fieldwork_team == "inbo") %>%
    filter(lsvi_measurement) %>%
    left_join(select(coordinates_2021, point_code, year, x_measured, y_measured), by = c("point_code", "year")) %>%
    anti_join(assessments_admin_inboveg_measured, by = c("point_code", "year")) %>%
    unique() %>%
...
assessments_admin_other <- assessments_admin_basic %>%
    anti_join(assessments_admin_inboveg_measured, by = c("point_code", "year")) %>%
    anti_join(assessments_admin_notinboveg_measured, by = c("point_code", "year")) %>% 
    filter(fieldwork_team == "inbo") %>%
    rename(assessment_date_admin = assessment_date)
...
check_unique_point_code <- assessments_admin_inboveg %>%
    group_by(point_code, assessment_date_admin) %>%
...
check_missing_admin <- visits_mhq_terr_wide %>%
    anti_join(assessments_admin_basic, by = c("db_ref", "year")) %>%
...
check_missing_inboveg <- assessments_admin_inboveg %>%
    filter(lsvi_measurement) %>%
    anti_join(visits_mhq_terr_wide, by = c("db_ref", "year")) %>%
    filter(type_observed != "91E0_sf") %>%
    mutate(coordinates_available = !is.na(x_measured))

# 	1381553 --> wn_05_double
# 	2003969 --> niet in admin
# wn_21 --> ,iet in admin
...
check_missing_inboveg %>%
    arrange(assessment_date_admin) %>%
    select(point_code, db_ref, assessment_date_admin, type_observed, lsvi_measurement, coordinates_available) %>%
    kable() %>%
    kable_styling()
...
check_missing_admin %>%
    select(db_ref, user_reference, recording_givid, assessment_date, square) %>%
...
```{r}
check_type_observed %>%
    select(point_code, db_ref, assessment_date_inboveg, type_observed, square) %>%
    kable() %>%
    kable_styling()
```
...
refpoints_assessed_inbo <- refpoints_assessed %>%
    left_join(assessments_admin_inboveg, by = "point_code") %>%
    distinct() %>%
    mutate(distance = sqrt((x_measured - x)^2 + (y_measured - y)^2))

check <- refpoints_assessed_inbo %>%
    filter(is.na(assessment_date))

check_missing_refpoint <- assessments_admin_inboveg %>%
    anti_join(refpoints_assessed, by = "point_code")
...
```{r}
refpoints_assessed_inbo <- refpoints_assessed_inbo %>%
    mutate(x_measured = ifelse(point_code %in% check_coordinates$point_code, x, x_measured),
           y_measured = ifelse(point_code %in% check_coordinates$point_code, y, y_measured)) %>%
    mutate(distance = sqrt((x_measured - x)^2 + (y_measured - y)^2))

#! Aanpassen in INBOVEG
```
...
```{r}
mhq_terr_assessments_anb <- mhq_terr_assessments %>%
    filter(fieldwork_team == "anb")

unique(assessments_admin_anb$info_status_fieldwork)

coordinates_anb <- 

    

assessments_anb <- assessments_admin_anb %>%
    left_join(type_observed_plot) %>%
    mutate(grts_ranking_draw = plot_id) %>%
...
assessments_admin_anb_new <- assessments_admin_anb %>%
    anti_join(mhq_terr_assessments_anb, by = c("grts_ranking_draw"))

assessments_admin_anb_update <- assessments_admin_anb %>%
    semi_join(mhq_terr_assessments_anb, by = c("grts_ranking_draw")) %>%
    anti_join(mhq_terr_assessments_anb, by = c("grts_ranking_draw", "assessment_date"))

mhq_terr_assessments_anb_keep <- mhq_terr_assessments_anb %>%
    anti_join(assessments_admin_anb_update, by = c())
...
check <- refpoints_assessed_anb_ortho %>%
    group_by(point_code, type_target) %>%
    filter(n() > 1) %>%
    ungroup()
...
orthocontrole_2021 <- bind_rows(
  ...
  filter(!orthocontrole) %>%
    mutate(is_present = ifelse(opmerking == "ontoegankelijk", NA, FALSE),
           is_type_target = NA,
           type_observed = NA,
...
refpoints_assessed_all <- refpoints_assessed_inbo %>%
    ...
    select(point_code, is_centroid, legacy_site, sac, grts_ranking, grts_ranking_draw, type_target_all, type_observed, x, y, fieldwork_team, db_ref, assessment_source, assessment_date, inaccessible,  not_measurable, lsvi_measurement, x_measured, y_measured, user_reference, recording_givid, distance, point_type) %>%
...
In 2021 inbo started with a new sampling strategy. When the target type is not observed in a sampling unit, we allow it to be replaced by the sampling unit that does contain the target type with the lowest grst-ranking, within the same habitatmap polygon. Legacy sampling units are not replaced.
