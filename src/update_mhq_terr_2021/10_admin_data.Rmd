...
su_duinen_2160_2180 <- read.csv2("../../data/mhq_terr_cyclus1_2160_2180_inbo.csv") %>%
    mutate(habitattype = as.character(habitattype))

su_duinen_cyclus2 <- read.csv2("../../data/mhq_terr_cyclus2_duinen_inbo.csv") %>%
    filter(type_meetpunt == "nieuw meetpunt")

su_terr_cyclus2 <- read.csv2("../../data/mhq_terr_cyclus2_inbo.csv") %>%
    filter(type_meetpunt == "nieuw meetpunt")

su_inbo_2022 <- su_duinen_2160_2180 %>%
...
# dubbele_invoer <- c("V2022051311343665", "IV2022051311373024", "IV2022051311475760", "IV2022051311493921", "IV2022092215204159", "IV2022090915325432", "IV2022051311343665", "IV2022091211044032")
...
# test <- visits_mhq_terr_wide %>%
#     dplyr::group_by(db_ref, user_reference, recording_givid, assessment_date, x_measured, y_measured, plot_type) %>%
...
# visits_mhq_terr_wide_1 <- visits_mhq_terr_wide %>%
#     filter(!reassessed)

# NOG aanpassen en nakijekn
assessments_admin_basic <- assessments_admin_basic %>%
    mutate(assessment_date = if_else(db_ref == "311262" & assessment_date == "2016-01-01", as.Date("2015-07-28"), assessment_date), #aangepast in mhq terr google sheet
           assessment_date = if_else(db_ref == "300649", as.Date("2016-08-31"), assessment_date), #aangepast in mhq terr google sheet
           assessment_date = if_else(db_ref == "1276081", as.Date("2020-08-21"), assessment_date), #aangepast in mhq terr google sheet
           assessment_date = if_else(db_ref == "358577", as.Date("2020-08-23"), assessment_date)) %>% #aangepast in mhq terr google sheet
    filter(db_ref != "8557745") #geen volledige opname

visits_mhq_terr_wide <- visits_mhq_terr_wide %>%
    mutate(db_ref = ifelse(db_ref == "11413617", "dh_01", db_ref)) %>%
    filter(db_ref != "2534622") # geen volledige opname
...
```{r}
check_type_observed %>%
    select(point_code, db_ref, assessment_date, type_observed, square) %>%
    kable() %>%
    kable_styling()
```
