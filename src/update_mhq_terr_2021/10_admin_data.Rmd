...
terr_measurements_validate <- refpoints_assessed_all %>%
    filter(lsvi_measurement) %>%
    filter(!is.na(type_observed)) %>%
    select(fieldwork_team, point_code, type = type_observed, measurement_date = assessment_date, db_ref, user_reference, recording_givid, distance, point_type, distance_centroid) %>%
...
terr_measurements_validate_until2020 <- terr_measurements_validate %>%
    left_join(grts_max_ranking, by = c("type", "sac")) %>%
    filter(year(measurement_date) <= 2020) %>%
    mutate(valid_type = grts_ranking_draw <=  max_ranking,
           valid_distance = is.na(distance) | distance <= 110.1,
           valid_distance_legacy = ! ((is.na(distance) | distance > 1) & legacy_site),
           valid_replacement = valid_distance_legacy & valid_distance) 

check_type <- terr_measurements_validate_until2020 %>%
    filter(!valid_type) %>%
    select(point_code, sac, grts_ranking_draw, max_ranking, measurement_date, type, valid_type)

check_distance <- terr_measurements_validate_until2020 %>%
    filter(!valid_distance) %>%
    select(point_code, sac, grts_ranking_draw, max_ranking, measurement_date, type, distance, valid_distance)

check_distance_legacy <- terr_measurements_validate_until2020 %>%
...
The next shows the measurements for which the second condition is not met.
...
terr_measurements_validate_from2021 <- terr_measurements_validate %>%
    left_join(grts_max_ranking_2021, by = c("type", "sac")) %>%
    filter(year(measurement_date) >= 2021) %>%
    mutate(valid_type = grts_ranking_draw <=  max_ranking,
           valid_centroid = distance_centroid >= 1,
           valid_distance_legacy = ! ((is.na(distance) | distance > 1) & legacy_site),
           valid_replacement = valid_distance_legacy) 

check_type <- terr_measurements_validate_from2021 %>%
    filter(!valid_type) %>%
    select(point_code, sac, grts_ranking_draw, max_ranking, measurement_date, type, valid_type)

check_centroid <- terr_measurements_validate_from2021 %>%
    filter(!valid_centroid) %>%
    select(point_code, sac, grts_ranking_draw, max_ranking, measurement_date, type, distance, valid_distance)
