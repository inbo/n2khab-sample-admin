...
refpoints_legacy_todo_selected <- refpoints_legacy_todo %>%
  ...
  mutate(ranking_rel = rank(grts_ranking_draw),
         ...
         y_coord = y_orig) %>%
  filter(selection) %>%
  mutate(type = "existing PINK PQ: do not replace",
         phab = 100,
         n_type_target = 1,
  ...
  st_join(habitatmap_pol) %>%
  st_drop_geometry() %>%
  select(sampling_unit_code, point_code, sac, type_target, phab, n_type_target, type_target_all, grts_ranking, grts_ranking_draw, ranking_rel, x = x_coord, y = y_coord, polygon_id, description_orig, type)
...
mhq_refpoints_new_inbo <- mhq_refpoints_todo_potential %>%
  ...
  # st_join(habitatmap_pol) %>%
  # st_drop_geometry() %>%
  mutate(type = "new sampling unit: replace within polygon if type target is not present") %>%
  select(sampling_unit_code, point_code, sac, type_target, phab, n_type_target, type_target_all, grts_ranking_draw, ranking_rel, x = x_coord, y = y_coord, polygon_id, description_orig, type) %>%
...
mhq_refpoints_new_inbo_polygons %>%
  ...
  st_write("../../output/polygons_todo_inbo_versie2021-04-29.shp", driver = "ESRI Shapefile",
         delete_layer = TRUE)
...
write_csv2(mhq_replacements, "../../output/replacements_inbo_versie2021-04-29.csv")
...
write_csv2(max_grts_ranking, "../../output/maxranking_inbo_versie2021-04-29.csv")
...
```{r}
mhq_refpoints_inbo_todo <- refpoints_legacy_todo_selected %>%
  bind_rows(mhq_refpoints_new_inbo) %>%
  arrange(type_target, sac, grts_ranking_draw) %>%
  select(-grts_ranking)

mhq_refpoints_inbo_todo %>%
  write_csv2("../../output/samplingunits_todo_inbo_versie2021-04-29.csv")
```
