...
# Generation of table with reference points for all mhq terrestial sampling units 

The sampling units for the monitoring habitat quality (mhq) programme [Westra et al., 2014](https://pureportal.inbo.be/portal/files/4339795/Westra_etal_2014_MonitoringNatura2000Habitats.pdf) were drawn based on the grts-method. The mhq sampling units were stored in different files. Here we will generate one standardized table containing all mhq sampling units including legacy sites. Legacy sites are existing samples that were included in mhq.
...
- refpoint_code: a unique code for the reference point of each sampling unit; this is the point in which it is evaluated wether the target type is present or not 
...
- grts_ranking: ranking number of the grts master grid cell in which the sampling_unit is located
...
We also bring together some variables related to the sample selection. These variables will only be used for quality contol within the mhq programme.
...
sample_forest_orig <- read_sf("../../mhq_sample-admin_data/original/sample/sample-locations/bos_hab", "meetnet_bos_versie20150303", crs =31370, quiet = TRUE) 
...
When a sampling unit was replaced with a legacy site, grts_ranking_draw of the legacy site was set to the grts_ranking of the original sampling unit. Often these legacy sites are not located within the same grts-cell as the sampling unit it replaces. When this happens, the grts_ranking differs from grts_ranking_draw.
...
The Flemish forest inventory (VBI) consists of random systematic sample on a 500m x 1000m grid covering the whole of Flanders. The VBI sampling units are measured in the same way as the mhq forest type sampling units and can therefore be used in mhq. However, until recently the habitat type in the the VBI sampling unit was not determined. Therefore we have to assume that the type corresponds to the habitatmap. We make an overlay between the centroids of the VBI sampling untis and [habitatmap_terr](https://zenodo.org/record/3540740) and we select all VBI sampling units that are located in polygons with at least a phab >= 50 for a forest type. Since 2020 (the start of the 3rd monitoring cycle), the type in the centroid of VBI sampling units is determined during the field assessments.
...
mhq_terr_samples_details <- samples %>%
    ...
    select(scheme, sampling_unit_code, point_code, type, grts_ranking, is_centroid, grts_ranking_draw,  legacy_site, x, y, phab, sac, certain, description_orig, year_planned, db_ref) %>%
    arrange(scheme, type, grts_ranking)

mhq_terr_samples <- mhq_terr_samples_details %>%
    select(scheme, sampling_unit_code, point_code, type, grts_ranking, is_centroid, grts_ranking_draw,  legacy_site, x, y)

write_vc(mhq_terr_samples, file = "mhq_terr_refpoints", root = "../../data", sorting = c("type", "sampling_unit_code"), strict = FALSE)

write_vc(mhq_terr_samples_details, file = "mhq_terr_refpoints_details", root = "../../data", sorting = c("type", "sampling_unit_code"), strict = FALSE)
...
The table is exported as 'mhq_terr_samples'.
