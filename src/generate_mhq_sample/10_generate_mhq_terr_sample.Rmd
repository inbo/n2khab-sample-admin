...
The original data files which are used in the scripts are stored in [this google drive folder](https://drive.google.com/open?id=11rStbOup5DFmHP8FQOe_KCOSkXJIwXx0) which can be accessed within INBO.   

# Generation of table with all mhq samples 

The mhq samples (ref) were stored in different files. Here we will generate one standardized table containing all mhq sampling units including legacy sites. Legacy sites are existing sampling units that were included in mhq.
