# Query fieldmap forest hab

```{r}

db_fieldmap_forest_hab <- "../../mhq_sample-admin_data/field-data/fieldmap/database_forest_2020-06-04/FIELDMAPDATA_BOSHAB_F_V2020_QC202004.FDB"

p <- paste("DRIVER={Firebird/InterBase(r) driver};UID=SYSDBA;PWD=masterkey; DBNAME=", db_fieldmap_forest_hab)

conn <- odbcDriverConnect(p)

query_test <- "SELECT IDPLOTS, SERIES, PLOT_ID, HAB, HAB_SUBTYPE, EXTRA_SUBTYPE, STATUS_FIELDWORK, INFO_STATUS_FIELDWORK, VEG_DONE, REG_DONE, DENDRO_DONE, INFO_SPRINGVEG, SPRINGVEG, REMARK, REMARK4 FROM GRID_POINTS"

test <- sqlQuery(conn, query_test)

test <- sqlColumns(conn, "GRID_POINTS")

data2 <- sqlQuery(conn, query_test)
...
;"
 ...
 gridpointsOrig <- sqlQuery(connectieDB, query_gridpoints)
  gridPoint <- gridpointsOrig %>%
    select(sample_id = SingleID, asses_date = Add_date, grts_ranking = RANKING, status_fieldwork = Value1, info_status_fieldwork = Value1.1, Remark) %>%
...
get_observed_hab <- function(db = db_fieldmap_open_hab){
  ...
  habObservedOrig <- sqlQuery(connectieDB, query_HabTypeObserved, stringsAsFactors = FALSE)
  ...
  habObserved <- habObservedOrig %>%
    rename(plot_id = IDPlots,
           ...
           type_observed_circle = Value1,
           date_standdescription = Add_date) %>%
...
get_measurements_square <- function(db = db_fieldmap_open_hab){
  ...
  coverPlots <- coverPlotsOrig %>%
    ...
    mutate(type_observed_square = ifelse(nchar(type_observed_square) %in% c(7, 8), #dus een habitatsubtype
                                         sub(" ", "_", type_observed_square),
...
get_coordinates <- function(db = db_fieldmap_open_hab){
  ...
  coordinates <- sqlQuery(connectieDB, query_coord) %>%
      select(location_id = SingleID, x_measured = X_m, y_measured = Y_m) %>%
...
sample_status <- get_status(db = db_fieldmap_forest_hab)
