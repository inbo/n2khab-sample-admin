...
get_type_observed_openhab <- function(db = db_fieldmap_openhab){
  ...
  type_observed <- type_observed_circle_orig %>%
    full_join(type_observed_square_orig, by = c("plot_id", "segment_id")) %>%
...
# uitgezonderd bedekking van struik+boomlaag worden alle bedekkingen via Tansley-schaal bepaald
  ...
  structure_var_cover <- structure_vars %>%
      ...
      select(plot_id, structure_var, cover_mean = Shrub_and_Treelayer_18m)
...
get_date_openhab <- function(db = db_fieldmap_openhab){
  ...
  Observer_date.Starting_time as date
  FROM Observer_date"
...
connect_db <- odbcDriverConnect(paste("DRIVER={Firebird/InterBase(r) driver};UID=SYSDBA;PWD=masterkey; DBNAME=", db_fieldmap_forest_hab))
