...
admin_anb <- sample_status %>%
    ...
    mutate(assessment_source = ifelse(status_fieldwork == "ja", "field assessment",
           ...
           location_change = ifelse(lsvi_measurement & !is.na(lsvi_measurement),
                                    ...
                                    NA)) %>%
    distinct(point_code, assessment_source, inaccessible, assessment_year, assessment_date, lsvi_measurement, completed, location_change) 
...
sample_assessments_anb <- sample_admin_anb %>%
    left_join(assessment_anb, by = c("sampling_unit_code", "point_code", "grts_ranking_draw", "sac", "type_target")) %>%
...
mhq_refpoints_validity <- sample_assessments_update2 %>%
    ...
    filter(!is.na(scheme)) %>%
    select(scheme, sampling_unit_code, point_code, date = assessment_date, is_valid)
...
mhq_terr_assessments <- sample_assessments_update2 %>%
    select(point_code, type_target, is_type_target, type_observed, inaccessible, assessment_date, assessment_source, x, y)
