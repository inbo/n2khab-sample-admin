...
segments <- header_mhq %>%
  ...
  mutate(data_imported = recording_givid %in% recordings_mhq$recording_givid) %>%
    ...
    mutate(is_type_target_plot = sum(is_type_target) > 0,
           ...
           cover_description = str_c(str_c(segment_weight, "% ", hab_observed), collapse =  " + ")) %>%
    ungroup() %>%
    ...
    mutate(segment_center = ifelse(n_segments <= 1, TRUE,
                                   ifelse(is_type_target_plot, is_type_target, 
                                          ifelse(is_main_type_target_plot, is_main_type_target, 
                                                 str_sub(hab_observed,1,1) == "6")))) %>%
    group_by(plot, sample_id, date) %>%
           mutate(n_center = sum(segment_center)) %>%
    ungroup()
