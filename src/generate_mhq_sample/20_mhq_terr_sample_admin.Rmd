...
export <- "export_fieldmap_2020-06-10"
...
admin_anb <- sample_status %>%
    ...
    mutate(assessment_source = ifelse(status_fieldwork == "ja", "field assessment",
           ...
           location_change = ifelse(lsvi_measurement & !is.na(lsvi_measurement),
                                    ...
                                    NA),
           assessment_year = ifelse(is.na(assessment_date), 
                                    as.numeric(format(date_status, "%Y")), 
                                    as.numeric(format(assessment_date, "%Y")))) %>%
    distinct(point_code, assessment_source, inaccessable, assessment_date, assessment_year, lsvi_measurement, completed, location_change) 
...
```{r}
# correction!! -> landuse geeft aan dat het een poel is --> dus 91E0_vc veranderen naar 'geen habitat'

type_observed_corr <- type_observed %>%
    mutate(type_observed_circle = ifelse(plot_id == 201689 & segment_id == 2, "geen habitat", type_observed_circle))
...
plot_types <- type_observed_corr %>% 
    ...
    mutate(is_type_target_plot = sum(is_type_target, na.rm = TRUE) > 0,
           is_main_type_target_plot = sum(str_c(type_observed, 1, 4) == main_type) > 0,
...
assessment_anb <- samples_grts %>%
    ...
    left_join(max_ranking, by = c("type_observed", "sac")) %>%
   # left_join(measurement_completed, by = c("point_code")) %>%
    mutate(is_type_target = type_observed == type,
        allow_other_type = ifelse(is_type_target_plot, NA, (grts_ranking_draw <= max_grts_ranking_draw) & !is.na(max_grts_ranking_draw))) %>%
    select( point_code, sampling_unit_code, grts_ranking_draw, type_target = type, type_observed, type_observed_cover = type_cover, is_type_target, allow_other_type, type_cover_circle, type_cover_square, n_type_observed, n_habtarget, n_center, max_grts_ranking_draw)
...
check_allow_other_type <- assessment_anb %>%
    left_join(select(admin_anb, point_code, lsvi_measurement)) %>%
    filter(lsvi_measurement) %>%
    filter(!is_type_target) %>%
    filter(!allow_other_type) %>%
