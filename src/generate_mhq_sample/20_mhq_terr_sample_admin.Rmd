...
mhq_assessments_summary <- sample_assessments_update %>%
    ...
    filter(sample_source != "mhq_assessment") %>%
    filter(!is.na(is_type_target)) %>% # sommige inbo measured plots zijn nog niet ingevoerd in inboveg
...
check <- n_distinct(mhq_assessments_summary$sample_id) == nrow(mhq_assessments_summary)
check_n <- mhq_assessments_summary %>%
    group_by(sample_id) %>%
    mutate(n = n()) %>%
    ungroup()

write_vc(mhq_assessments_summary, root = "../../n2khab_mhq_data" , file = "mhq_assessments", sorting = c("type_target", "grts_ranking"))
