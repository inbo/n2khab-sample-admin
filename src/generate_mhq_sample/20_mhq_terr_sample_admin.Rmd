...
export <- "export_fieldmap_2020-06-17"
...
```{r}
dendro_records <- trees_a3a4 %>%
    mutate(dendro_record = TRUE) %>%
    mutate(point_code = str_c(plot_id, "_1")) %>%
    distinct(point_code, dendro_record)
    
veg_records <- cover_species %>%
    mutate(veg_record = TRUE) %>%
    mutate(point_code = str_c(plot_id, "_1")) %>%
    distinct(point_code, veg_record)

    
check_admin <- admin_anb %>%
    left_join(dendro_records, by = "point_code") %>%
    mutate(dendro_record = ifelse(db == "foresthab" & is.na(dendro_record), FALSE, dendro_record)) %>%
    left_join(veg_records, by = "point_code") %>%
    mutate(veg_record = ifelse(is.na(veg_record), FALSE, veg_record)) %>%
    left_join( distinct( assessment_anb, point_code, type_observed, type_cover_circle), by = "point_code")
```
...
#dubbele coödinaten voor 4 plots --> te checken
...
check_coordinates_double_or_error <- check_n %>%
    filter(n > 1 | x_assessment > 100000000000) %>%
...
write.table(check_coordinates_double_or_error, "../../output/check_coordinates_double.txt")
...
sample_admin_anb <- sample_admin_anb %>%
    mutate(x_assessment = ifelse(x_assessment > 100000000000, NA, x_assessment),
           y_assessment = ifelse(y_assessment > 100000000000, NA, y_assessment),
           distance_from_refpoint = ifelse(x_assessment > 100000000000, NA, distance_from_refpoint)) %>%
...
```{r}
check_assessment_date <- sample_assessments_anb %>%
    filter(assessment_source == "field assessment") %>%
    select(point_code, sampling_unit_code, assessment_source, lsvi_measurement, type_target, type_observed, assessment_date, completed)
```
