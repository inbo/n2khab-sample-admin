...
In the mhq sample adminstration table we want to document assessments and measurements of the mhq samples. 
An assessment of a mhq sample is seen as evaluation whether or not a sample is valid. 
A mhq sample is valid if the target type (= the type for which the sample was selected) corresponds with the observed type in the center of the grid cell. 
A location can contain different samples, with each sample having a different target type. 
This is the case when the sample frame of the different target types overlap. 
However, only one of the samples on a single location can be valid, as only one type can be observed in a location. 

Two types of assessments occur: field assessment and othophoto assessment. 
Based on othophoto assessment we can detect samples that are not valid (for example when a heath sample is located within a forest), but an field assessment is required to be sure that a sample is valid.
The variable 'assessment_source' indicates the type of assessment. If NA no assessment has taken place yet. 

In some cases locations are temporary or permanently inaccessable and assessments are not possible. 
This indicated by the variable 'inaccessable'. We distinguish 'short term' inaccessability and 'long term' inaccessability.

After assessment, a location is measured when one of the samples which it contains is valid. 
...
In certain conditions, a location can also be lsvi_measurement when the observed type is different than one of the target types for this location (and consequently none of the samples are valid). 
This is allowed when the grts-ranking of the location is smaller than the maximum grts-ranking in the sample set for the observed type. 
It occurs when the sample frame of the observed type is not correct and excludes the location mentioned above. 
When this is the case we will create a new (valid) sample for this location with the appropriate target type. 
This new sample will replace the sample in the original sample set with the highest grts-ranking.                 

The detection rate of some types is very low. 
Therefore in some cases we allow changing the location of a sample in the direct surrounding (based on a protocol) in order to make it a valid sample. The sample administration table will also document these location changes. 
Finally we will also document wether measurements are complete or wether some variables are missing in the database. 


```{r}
samples_grts <- read_vc(file = "mhq_terr_refpoints_details", root = "../../data") 
```
...
INBO documents the administration of mhq sample assessments in separate files. From this file we van determine the assessment status, the assessment year, wether a location is lsvi_measurement and wether a location is changed. 
There is one file for grassland (excluding 6510) and marsh types and one file for coastal dune types. 
Later we will propose a standardised file for all mhq samples.  

#### Grassland habitat (excluding 6510) and marshes

This file originally contains one record for each sample. 
However it is more convenient and it avoids conflicts when assessment status and measurement status is documented on the location level. 
That is because a location can only have one assessment status or measurement status. 
Therefore we convert the file to a table with one record for each location (that contains samples). 
For this we use the point_code of the location.  
...
admin_inbo_grassland_marshes <- admin_inbo_grassland_marshes_correct %>%
    mutate(NoAccess = ifelse(ID == "2388102_6230_hn", 1, NoAccess)) %>%
    mutate(inaccessable = ifelse(NoAccess == 1 & !is.na(NoAccess), "long term", NA),
           assessment_source = ifelse(bezocht > 1000 & is.na(inaccessable), "field assessment", NA),
    ...
    select(sampling_unit_code = ID, grts_ranking_draw = Ranking, assessment_source, assessment_year, inaccessable, lsvi_measurement, location_change, x_assessment, y_assessment)
...
- check: one location is assessed twice
...
admin_inbo_grassland_marshes_location <- admin_inbo_grassland_marshes %>%
    ...
    mutate(assessment_source_loc = ifelse("field assessment" %in% assessment_source, "field assessment",
            ...
            inaccessable_loc = ifelse("long term" %in% inaccessable, "long term", NA)) %>%
    ungroup() %>%
    group_by(grts_ranking_draw, assessment_source_loc, inaccessable_loc) %>%
    ...
    rename(assessment_source = assessment_source_loc, inaccessable = inaccessable_loc)
...
The admin file for coastal dunes contains one record for each location. 
The file does not contain a unique code for each location. 
However we can create the variable 'db_ref' which can be used to link the location to the mhq_sample_refpoints and to the INBOveg data.
...
This way we can evaluate if a sample is valid. 
We also check if the measurements are complete, i.e. vegetation composition is lsvi_measurement in the square plot and structure variables are lsvi_measurement in the circle plot. 
...
When the observed type is different from (one of) the target type(s), the plot can be recorded when the GRTS ranking of the sample location is smaller than the largest GRTS ranking in the set of samples for the observed type. 
...
Some recorded plots contain different habitat types or other cover types. 
However it is not explicitely recorded which of the cover types is located in the center of the plot. 
Yet this is essential information to determine if a sample is valid. 
Therefore, when a plot contains different cover types we assume that the plot center is located in the type recorded in the square plot that corresponds to (one of) the target type(s). 
When none of the target habitat types are observed we assume that the plot center is located in the type that belongs to the same main type as (one of) the target habitat types. We apply these rules and check of we select one type for each plot this way.
...
plot_types <- plots %>%
  left_join(classif_mhq, by = c("recording_givid", "survey")) %>%
    ...
    inner_join(select(samples_grts, point_code, sampling_unit_code, type), by = "point_code") %>%
    left_join(select(types, type, main_type), by ="type") %>%
...
plot_types_check <- plot_types %>%
    ...
    select(recording_givid, sampling_unit_code, plot_bevat_doelhabitat = is_type_target_plot, date, hab_target = type, type_observed, cover = type_cover, n_type_observed) %>%
...
assessment_inbo <- samples_grts %>%
    ...
    select( point_code, sampling_unit_code, grts_ranking_draw, assessment_date = date, assessment_year,completed, type_target = type, type_observed, type_observed_cover = type_cover, is_type_target, allow_other_type, type_cover_circle, type_cover_square, n_type_observed, n_habtarget, n_center, max_grts_ranking_draw)
...
Some samples are lsvi_measurement altough the observed type is different than the target type and the grts-ranking is larger than the maximum grts-ranking in the sample set of the observed type (allow_other_type = FALSE).
...
We join by the point_code, because and assessment is done for all target types (samples) in a location. However we only select the samples for the types that were assigned to INBO field work team: `r types_inbo`. 

```{r}
sample_admin_inbo <- samples_grts %>%
    filter(type %in% types_inbo) %>%
    #select(-db_ref) %>%
    full_join(admin_inbo, by = c("point_code", "grts_ranking_draw")) %>%
...
sample_assessments_inbo <- sample_admin_inbo %>%
    left_join(assessment_inbo, by = c("sampling_unit_code", "point_code", "grts_ranking_draw", "assessment_year")) %>%
    mutate(is_type_target = ifelse(assessment_source %in% c("field assessment", "orthophoto assessment"),
                                   ifelse(!lsvi_measurement, FALSE, is_type_target),
                                   is_type_target),
           type_target = ifelse(is.na(type_target), as.character(type), type_target),
           fieldwork_team = "inbo") %>%
    select(scheme, sampling_unit_code, type_target, point_code, sac, grts_ranking, grts_ranking_draw, year_planned, phab, description_orig, certain, assessment_source, assessment_year, inaccessable, lsvi_measurement, completed, location_change, is_type_target, type_observed,  allow_other_type, type_cover_circle, type_cover_square, x_orig = x, y_orig = y, x_assessment, y_assessment, distance_from_refpoint, fieldwork_team, db_ref, is_centroid, legacy_site)
...
check_position_change <- sample_admin_inbo %>%
    ...
    mutate(distance_from_refpoint = round(sqrt((x_assessment- x)^2 + (y_assessment- y)^2), 1)) %>%
    select(sampling_unit_code, type, point_code, lsvi_measurement, location_change,  distance_from_refpoint)

#some of the coordinates in the PINK sample are wrong --> to check

```

According to the protocol locations can only be changed within a distance of 100 meters. For following samples, the distance from the lsvi_measurement location to the original sample location is larger than 100 meters.
...
When no position change is mentioned inthe admin table, we expect the lsvi_measurement location to be close to the original sample location. However, the following locations show a distance larger than 1 meter. Which is the tolerated location error? From which distance we should set 'location_change' to TRUE?
...
hab_observed <- read.csv2(str_c("../../mhq_sample-admin_data/field-data/fieldmap/", export, "/hab_observed.csv"), stringsAsFactors = FALSE)
measurements_6510_circle <- read.csv2(str_c("../../mhq_sample-admin_data/field-data/fieldmap/", export, "/measurements_6510_circle.csv"), stringsAsFactors = FALSE)
measurements_heath_circle <- read.csv2(str_c("../../mhq_sample-admin_data/field-data/fieldmap/", export, "/measurements_heath_circle.csv"), stringsAsFactors = FALSE)
measurements_square <- read.csv2(str_c("../../mhq_sample-admin_data/field-data/fieldmap/", export, "/measurements_square.csv"), stringsAsFactors = FALSE)
sample_status <- read.csv2(str_c("../../mhq_sample-admin_data/field-data/fieldmap/", export, "/sample_status.csv"), stringsAsFactors = FALSE)
observer_date <- read.csv2(str_c("../../mhq_sample-admin_data/field-data/fieldmap/", export, "/observer_date.csv"), stringsAsFactors = FALSE)
...
date_overview <- hab_observed %>%
     mutate(date_standdescription = as.Date(date_standdescription, format = "%Y-%m-%d"),
            point_code = str_c(as.character(plot_id), "_1")) %>% 
    select(point_code, date_standdescription) %>%
    unique() %>%
    left_join(observer_date, by = "point_code") %>%
    ...
    select(point_code, assessment_date, assessment_year)

admin_anb <- sample_status %>%
    mutate(point_code = str_c(as.character(grts_ranking), "_1")) %>%
    left_join(date_overview, by = "point_code") %>%
    mutate(assessment_source = ifelse(status_fieldwork == "ja", "field assessment",
                                  ifelse(status_fieldwork == "nee", NA, NA)),
           inaccessable = ifelse(!is.na(info_status_fieldwork) & info_status_fieldwork == "Geen opname : tijdelijk nt toeg", "short term", 
           ...
           lsvi_measurement = ifelse(assessment_source == "field assessment",
                                ...
                                NA),
           lsvi_measurement = ifelse(inaccessable %in% c("short term", "long term"), FALSE, lsvi_measurement),
           assessment_source = ifelse(inaccessable %in% c("short term", "long term"), NA, assessment_source),
           ...
           location_change = ifelse(lsvi_measurement & !is.na(lsvi_measurement),
                                    ...
                                    NA)) %>%
    distinct(point_code, assessment_source, inaccessable, assessment_year, lsvi_measurement, completed, location_change) 
...
type_square <- measurements_square %>%
    ...
    select(type_observed = type_observed_square, point_code) %>%
    unique() %>%
    mutate(type_cover_square = str_c(type_observed))
    
assessment_anb <- type_square %>%
    left_join(type_circle, by = "point_code") %>%
    left_join(select(samples_grts, sampling_unit_code, point_code, grts_ranking_draw, type, sac), by = c("point_code")) %>%
    left_join(max_ranking, by = c("type_observed", "sac")) %>%
    rename(type_target = type) %>%
    mutate(main_type = str_sub(type_target, 1, 4),
          type_observed = ifelse(is.na(type_observed) & !is.na(type_cover_circle),
    ...
    ungroup() %>%
    mutate(allow_other_type = ifelse(is_type_target_plot, NA, (grts_ranking_draw <= max_grts_ranking_draw) & !is.na(max_grts_ranking_draw))) %>%
    select(point_code, sampling_unit_code, sac, grts_ranking_draw, type_target, type_observed, type_observed_cover = type_cover, is_type_target, allow_other_type, type_cover_circle, type_cover_square, n_habtarget, max_grts_ranking_draw)
...
#dubbele coödinaten voor 4 plots

```

#### Check coordinates

Some samples have two different lsvi_measurement coordinates. For now, we select the lsvi_measurement coordinates that are closest to the original sample.

```{r}
check_coordinates_double <- check_n %>%
    filter(n > 1) %>%
    select(sampling_unit_code, point_code, assessment_source, lsvi_measurement, x, y, x_assessment, y_assessment, distance_from_refpoint)
...
For some samples the lsvi_measurement location is more tha 100 meters away from the original sample. In some cases the distance is extremely large and probably the point_code in the database is incorrect.

```{r}
check_distance <-  check_n %>%
    filter(distance_from_refpoint > 100) %>%
    select(sampling_unit_code, point_code, assessment_source, lsvi_measurement, distance_from_refpoint, x, y, x_assessment, y_assessment) %>%
    arrange(desc(distance_from_refpoint))
...
ANB also records in most cases the observed habitat type/land cover class when a sample is not lsvi_measurement.

```{r}

sample_assessments_anb <- sample_admin_anb %>%
    left_join(assessment_anb, by = c("sampling_unit_code", "point_code", "grts_ranking_draw", "sac")) %>%
    mutate(is_type_target = ifelse(assessment_source %in% c("field assessment", "orthophoto"),
           ...
           fieldwork_team = "anb") %>%
    select(scheme, sampling_unit_code, type_target, point_code, sac, grts_ranking_draw, grts_ranking, year_planned, phab, description_orig, certain, assessment_source, inaccessable, assessment_year, lsvi_measurement, completed, location_change, is_type_target, type_observed,  allow_other_type, type_cover_circle, type_cover_square, x_orig = x, y_orig = y, x_assessment, y_assessment, distance_from_refpoint, fieldwork_team, db_ref, is_centroid, legacy_site) 
...
```{r}
control <- read.csv2("../../mhq_sample-admin_data/original/admin/anb/PlotsMetDataAFGEWERKT.csv", stringsAsFactors = FALSE) %>%
    mutate(point_code = str_c(as.character(IDPlots), "_1")) %>%
    select(point_code, completed_check = AFGEWERKT)

check <- sample_assessments_anb %>%
    left_join(control, by = "point_code") %>%
...
sample_assessments <- bind_rows(
    ...
    mutate(type_target = factor(type_target, levels= levels(types$type)),
           ...
           is_valid = is_type_target & (distance_from_refpoint <= 110) & is.na(inaccessable))
```


## Create new sampling units when is_type_target = FALSE

When a location is measured and the observed type is different from the target type, we create a new sampling unit within the scheme of the observed type. 

The new sampling unit is valid when allow_other_type = TRUE and when the distance from the reference point is smaller than 110 meters


```{r}
new_sampling_units <- sample_assessments %>%
    filter(assessment_source == "field assessment" & !is.na(allow_other_type)) %>%
    filter(lsvi_measurement) %>%
    filter(!is_type_target) %>%
    filter(type_observed %in% types$type) %>%
    mutate(sampling_unit_code = str_c(grts_ranking_draw, "_", type_observed),
           ...
           phab = NA,
           is_type_target = TRUE,
           is_valid = allow_other_type & distance_from_refpoint <= 110,
           allow_other_type = NA,
...
We change the point_code of the locations for which the location was changed. 
This is done by adding adding 1 to the number at the end of the point_code. 

Next we create assessments for the original coordinates of the changed locations. It is assumed that for the changed locations, the target type is not present in the original location.
...
sample_units_change_location <-  sample_assessments_update %>%
    ...
    filter(distance_from_refpoint > 1) %>%
    filter(is_type_target) %>%
    mutate(temp_n = as.numeric(str_sub(point_code, -1)),
           point_code = str_c(grts_ranking_draw, "_", temp_n +1),
           x = x_assessment,
           y = y_assessment,
           is_centroid = FALSE,
           legacy_site = FALSE) %>%
...
mhq_terr_refpoint_new <- sample_assessments_update2 %>%
    ...
    select(scheme, sampling_unit_code, point_code, type = type_target, grts_ranking, is_centroid, grts_ranking_draw, legacy_site, x, y)

mhq_terr_refpoint_update <- mhq_terr_refpoint %>%
    bind_rows(mhq_terr_refpoint_new) %>%
    arrange(type, sampling_unit_code)
...
write_vc(mhq_terr_refpoint_update, file = "mhq_terr_refpoints_check_update", root = "../../data", sorting = c("scheme", "sampling_unit_code", "point_code"), strict = FALSE)

mhq_terr_refpoint_details <- read_vc(file = "mhq_terr_refpoints_details", root = "../../data")

mhq_terr_refpoint_details_new <- sample_assessments_update2 %>%
    anti_join(mhq_terr_refpoint_details, by = c("sampling_unit_code", "point_code")) %>%
    select(scheme, sampling_unit_code, point_code, type = type_target, grts_ranking, is_centroid, grts_ranking_draw, legacy_site, x, y, phab, sac, certain, description_orig, year_planned, db_ref)

mhq_terr_refpoint_details_update <- mhq_terr_refpoint_details %>%
    bind_rows(mhq_terr_refpoint_details_new) %>%
    arrange(type, sampling_unit_code)

write_vc(mhq_terr_refpoint_update, file = "mhq_terr_refpoints_details_check_update", root = "../../data", sorting = c("type", "sampling_unit_code", "point_code"), strict = FALSE)
```
...
mhq_refpoints_validity <- sample_assessments_update2 %>%
    ...
    filter(!is.na(scheme)) %>%
    select(scheme, sampling_unit_code, point_code, date = assessment_year, is_valid)

write_vc(mhq_refpoints_validity, file = "mhq_terr_refpoints_validity", root = "../../data", sorting = c("scheme", "sampling_unit_code", "point_code"))
```
...
### Create mhq_refpoints_assessments

The table mhq_refpoints_assessments gives an overview of all assessments (see below for example records). 

```{r}
mhq_refpoints_assessments <- sample_assessments_update2 %>%
    select(point_code, type_target, is_type_target, type_observed, inaccessable, assessment_date = assessment_year, assessment_source, x, y)
```

```{r}
mhq_refpoints_assessments %>%
