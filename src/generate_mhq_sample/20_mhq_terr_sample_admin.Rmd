...
sample_status <- sample_status %>%
    mutate(status_fieldwork = ifelse(plot_id == "4230578", "nee", status_fieldwork))
#---
...
For some samples for main types 6510, 9130 and 2330, the observed subtype is not specified. When this is the case we assume that the subtype corresponds to the target type.
...
plot_types <- type_observed_corr %>% 
    ...
    mutate(main_type = str_sub(type_target, 1, 4),
           ...
           type_observed = ifelse(type_observed == "9130" & main_type == "9130", as.character(type_target), type_observed),
           type_observed = ifelse(type_observed == "9130" & main_type != "9130", "9130_end", type_observed),
           is_type_target = type_target == type_observed,
           type_cover = type_observed_cover_circle,
           is_main_type_target = str_sub(type_observed, 1, 4) == main_type) %>%
    group_by(point_code) %>%
    mutate(is_type_target_plot = sum(is_type_target, na.rm = TRUE) > 0,
           is_main_type_target_plot = sum(str_sub(type_observed, 1, 4) == main_type) > 0,
           n_habtarget = n_distinct(type_target)) %>%
    ungroup()
