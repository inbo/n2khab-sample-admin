...
assessment_inbo <- samples_grts %>%
    ...
    select(location_id, sample_id, grts_ranking, assessment_date = date, assessment_year, type_target = type, type_observed, type_observed_cover = type_cover, is_type_target, allow_other_type, type_cover_circle, type_cover_square, n_type_observed, n_habtarget, n_center, max_grts_ranking)
...
Next, we join the sample table and the admin data. We join by the location_id, because and assessment is done for all target types (samples) in a location. However we only select the samples for the types that were assigned to INBO field work team. 
...
sample_assessments_inbo <- sample_admin_inbo %>%
    ...
    select(sample_id, type_target, location_id, grts_ranking, assessment_status, assessment_year, measured, location_change, is_type_target, type_target, type_observed,  allow_other_type, x_measured, y_measured, distance_from_sample)
