...
admin_inbo_grassland_marshes <- admin_inbo_grassland_marshes_correct %>%
    ...
    mutate(sample_status = ifelse(bezocht > 1000, "in situ assessment", "no assessment"),
           ...
           sample_status = ifelse(OrthoContr == 1 & !is.na(OrthoContr), "orthophoto assessment", sample_status),
...
assessment_inbo <- samples_grts %>%
    ...
    select(location_id, sample_id, grts_ranking, date_assessment = date, assessment_year, type_target = type, type_observed, type_observed_cover = type_cover, is_type_target, allow_other_type, type_cover_circle, type_cover_square, n_segments, n_habtarget, n_center)
...
check_position_change <- sample_assessments %>%
    filter(measured) %>%
...
export <- "export_fieldmap_2020-03-30"
...
admin_anb <- sample_status %>%
    ...
    mutate(sample_status = ifelse(status_fieldwork == "ja", "in situ assessment",
                                  ifelse(status_fieldwork == "nee", "no assessment", NA)),
           sample_status = ifelse(!is.na(info_status_fieldwork) & info_status_fieldwork == "Geen opname : tijdelijk nt toeg", "temporary inaccessable", 
                                  ifelse(info_status_fieldwork %in% c("Geen opname : permanent nt toegankelijk", "Geen opname : geen toelating"), "permanently inaccessable", sample_status)),
           sample_status = ifelse(!is.na(info_status_fieldwork) &  info_status_fieldwork == "Geen (volledig) terreinbezoek uitgevoerd", "no assessment", sample_status),
           measured = ifelse(sample_status == "in situ assessment",
           ...
           position_change = ifelse(is.na(Remark), FALSE,
                                    str_detect( Remark, "verlegd"))) %>%
    select(sample_id, location_id, sample_status, measured, assessment_year, position_change) %>%
    left_join(completed, by = "location_id")
