...
path_mhq_terr <- "../../data/mhq_terr/v2024"
...
assessments_admin_inbo <- read_csv2("../../data/admin_mhq_terr/voortgang_mhq_terrestrisch_2025-12-03.csv")
...
original_reference_points <- refpoints_assessed_all %>%
    semi_join(new_reference_points, by = c("point_code" = "point_code_old", "year")) %>%
    mutate(lsvi_measurement = FALSE,
           type_observed = NA,
           distance = NA,
           x_measured = NA,
...
terr_refpoints <- refpoints_assessed_all %>%
    mutate(x = ifelse(!is.na(x_measured),
                      round(x_measured, 2),
                      round(x, 2)),
           y = ifelse(!is.na(y_measured),
                      round(y_measured, 2),
                      round(y, 2))) %>%
