...
sampling_units_new <- su_duinen_cyclus2 %>%
...
mhq_terr_refpoints_anb_new <- mhq_terr_refpoints_anb_new %>%
    ...
    st_drop_geometry() %>%
    select(point_code, is_centroid, x = x_coord, y = y_coord)
...
assessments_measured <- assessments_admin_inbo_new %>%
    select(-x_exist, -y_exist, -dist, -assessment_date_exist) %>%
...
+ new inboveg records with no assessements records in the admin table
...
check_coordinates %>%
    select(point_code, db_ref, type_observed, x, y, x_measured, y_measured, distance, relocation, reassessment_1)
```
...
```{r, eval = FALSE}
heath_ortho_control_orig <- read_sf("../../data/design_cycle1/", "meetnet_heide_versie20140611_orthocontrole") %>%
    st_drop_geometry()
...
heath_ortho_control2_orig <- read_sf("../../data/design_cycle1/", "meetnet_heide_versie20140611_SelectieJaar4_5_6") %>%
    st_drop_geometry() %>%
...
# verplaatsing binnen gridcel of naar andere gridcel maar niet in centrum ervan 
...
check_new <- mhq_terr_assessments %>%
    semi_join(terr_assessments_new, by = c("point_code", "assessment_date"))
...
check <- mhq_terr_popunits %>%
    left_join(select(mhq_terr_assessments, point_code, type, assessment_date), by = c("point_code", "type"))
...
new_point_code <- terr_assessments_new %>%
    anti_join(mhq_terr_popunits, by = c("point_code")) %>%
    filter(is_present) 

new_type <-  terr_assessments_new %>%
    anti_join(mhq_terr_popunits, by = c("point_code", "type")) %>%
    semi_join(mhq_terr_popunits, by = "point_code") %>%
    filter(is_present) 

new_type_absent <- terr_assessments_new %>%
    semi_join(mhq_terr_popunits, by = c("point_code", "type")) %>%
    filter(!is_present)
...
%>%
    ...
    mutate(assessed = TRUE,
           measured = point_code %in% mhq_terr_measurements_update$point_code) %>%
...
```{r}
grts_max_ranking_type <- terr_refpoints %>%
